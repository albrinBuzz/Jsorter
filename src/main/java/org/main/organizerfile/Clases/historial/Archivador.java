package org.main.organizerfile.Clases.historial;
...
public class Archivador {
    ...
    public void serialiar() {
        String historial = userHome + slash + ".config" + slash + "Jsort" + slash + "historial.bin";
        File conf=new File(config);
        if (conf.exists()) {
            new File(historial).setWritable(true);
            try (ObjectOutputStream objectInputStream = new ObjectOutputStream(new FileOutputStream(historial))) {
                {
                ...
                new File(historial).setReadOnly();
            }
    ...
    public void deserializar(){
        String historial=userHome+slash+".config"+slash+"Jsort"+slash+"historial.bin";
        if (new File(historial).exists()){
            try (ObjectInputStream objectInputStream=new ObjectInputStream(new FileInputStream(historial))){
    ...
    public void crearLog(String direct) throws IOException {
        File logFile = new File(config+slash+"log.txt");
        if (!logFile.exists()) {
            logFile.createNewFile();
        }else {
            logFile.setWritable(true);
        }
        try (PrintWriter printWriter = new PrintWriter(new FileWriter(config+slash+"log.txt", true))) {
